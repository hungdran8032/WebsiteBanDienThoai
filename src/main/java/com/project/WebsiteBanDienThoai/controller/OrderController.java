...
//    public String getOrders(@RequestParam("month") Optional<Integer> month, Model model) {
...
//        List<Object[]> topProducts = orderDetailRepository.findTop3ProductsByOrderDateBetween(startDate, endDate, PageRequest.of(0, 3));
//
//        model.addAttribute("orders", orders);
...
//    }
    @GetMapping("/admin/orders")
    public String getMonthlyProductCount(Model model) {
        List<MonthlyProductCount> monthlyProductCounts = orderService.getMonthlyProductCount();
        model.addAttribute("monthlyProductCounts", monthlyProductCounts);
        return "/admin/orders";
    }
}